...
procedure darBaja(var a:archivo);
    ...
    procedure leer(var a:archivo; var rec:empleado);
    begin
        if(not eof(a))then
            read(a,rec)
        else
...
var
    numE:integer;
    last_rec:empleado;
    rec_eliminar:empleado;
begin
    write('Ingrese numero de empleado a eliminar: ');
    readln(numE);
    reset(a);
    seek(a, filesize(a)-1);
    read(a,last_rec);
    if(last_rec.numE <> numE)then begin
        seek(a, 0);
        leer(a,rec_eliminar);
        while((rec_eliminar.numE <> valor_alto)and(rec_eliminar.numE <> numE))do 
            leer(a,rec_eliminar);
        if(rec_eliminar.numE <> valor_alto)then begin
            seek(a, filepos(a)-1);
            write(a, last_rec);
            seek(a, filesize(a)-1); {me posiciono en la ultima posicion de nuevo}
            truncate(a); {pongo el EOF ahi}
            write('El empleado se elimino con exito!');
            writeln(' se cambio la posicion del ultimo empleado a la del eliminado.');
        end
        else
        ...
        seek(a, filesize(a)-1);
        truncate(a);
        writeln('El empleado estaba ultimo, se elimino con exito!')
    end;
    close(a);
  	writeln('======================================');
end;
